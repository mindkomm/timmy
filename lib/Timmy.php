...
class Timmy {
	...
	public function filter_image_downsize( $return, $attachment_id, $size ) {
        ...
        // Bail out if no width or height is set. This is the case for PDF files or other non-image
        // attachments, which don’t have an own width or height, but can have a thumbnail that can
        // be generated by WordPress.
        // PDF thumbnails will be generated by WordPress and not Timmy, if a thumbnail, medium or
        // large size is defined in the image configuration.
        if (!isset($meta_data['width']) && !isset($meta_data['height'])) {
            return $return;
        }

		$max_width  = $meta_data['width'] ?? null;
		$max_height = $meta_data['height'] ?? null;
