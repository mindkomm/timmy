...
## How does Timmy handle PDF thumbnails?

When you define a `thumbnail`, `medium` or `large` size key in your image configuration, then WordPress will generate PDF thumbnails for these sizes. 

These sizes are not handled by Timmy but by WordPress and will have different file names than images generated by Timmy. If you want to restrict the sizes generated by WordPress, you can use the [`fallback_intermediate_image_sizes`](https://developer.wordpress.org/reference/hooks/fallback_intermediate_image_sizes/) filter hook.

To access the PDF thumbnails, you can use [`wp_get_attachment_image_src()`](https://developer.wordpress.org/reference/functions/wp_get_attachment_image_src/).

If your installation doesn’t generate PDF thumbnails, it might also be that Ghostscript isn’t installed or if there’s an ImageMagick security policy present that restricts reading and writing PDF files.
